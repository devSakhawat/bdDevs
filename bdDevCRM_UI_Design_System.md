# 📘 bdDevCRM — UI Design System v2.0
...
3. [Tech Architecture — Frontend](#3-tech-architecture--frontend)
4. [Design Tokens](#4-design-tokens)
5. [Component Standards](#5-component-standards)
6. [Grid UX Standard](#6-grid-ux-standard)
7. [Form UX Standard](#7-form-ux-standard)
8. [Interaction Rules](#8-interaction-rules)
9. [Navigation UX](#9-navigation-ux)
10. [Empty State & Skeleton](#10-empty-state--skeleton)
11. [Responsive Rules](#11-responsive-rules)
12. [Permission UX](#12-permission-ux)
...
17. [File Architecture Reference](#17-file-architecture-reference)
...
এই document হলো bdDevCRM-এর **Single Source of Truth** — UI related সব সিদ্ধান্ত, pattern, standard এখানে documented। ভবিষ্যতে যেকোনো সময় ফিরে এসে বুঝতে পারবেন:
...
## 3. Tech Architecture — Frontend

### 3.1 TypeScript → esbuild → Bundle Pipeline
...
│   ├── navigation-service.ts  ← ROUTE_MAP, breadcrumb, active menu sync
...
### 3.4 Window Globals (exposed by bundle.ts)
...
**Rules:**
- Hover effects → `fast`
- Sidebar collapse, modal open → `base`
- Page transitions → `slow`
- **No heavy animation** — fast & subtle always
...
### 6.3 Grid Toolbar (Standard for ALL grids)
...
- Delete = always **last item**, always **red text**
- Delete click → **Confirm modal** (never direct delete)
- **Permission-based**: unauthorized items are **hidden** (not disabled)
- **Module-specific items** can be added:
  - Lead grid: "Convert to Student", "Assign to Agent"
  - Student grid: "Create Application", "Send Email"
  - Invoice grid: "Mark as Paid", "Send Reminder"

### 6.6 Grid Column Configuration Pattern
...
### 7.2 Type 3 — Grid/Form Toggle (Primary Pattern)
...
### 7.6 Form Dirty Check (form-guard)
...
- ❌ Never use browser default spinner
- ✅ Always skeleton or shimmer for data areas
- ✅ Button click → spinner **inside** the button

### 8.2 Toast Notification Rules
...
**Has:** Progress bar, close button, optional action button

### 8.3 Confirm Dialog Rules
...
- Generated from `ROUTE_MAP` in navigation-service.ts
...
**Rules:**
- Icon: relevant to the module (📋 leads, 👥 employees, 📄 invoices)
- Title: short, clear
- Description: helpful, guides user to action
- CTA button: primary, creates new record
- **Permission check**: if user can't create, hide CTA button

### 10.2 Grid Skeleton Loading
...
### 14.2 JavaScript/TypeScript Window Globals
...
2. **Empty TS files** — 5 components/services need JS→TS migration
3. **Grid Context Menu** — Enterprise must-have, 0% done
4. **Empty State** — Better UX, 0% done
5. **Form Validation Visual** — No standard, 0% done
...
### Phase 2: TS Migration (Core Components)
...
## 17. File Architecture Reference

### Final Target State (after all cleanup)
...
> **📌 ���ই document আপডেট করবেন যখনই:**
> - নতুন design decision নেবেন
> - Implementation status পরিবর্তন হবে
> - নতুন component pattern add হবে
> - Phase transition হবে
>
